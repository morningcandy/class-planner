...
| Apps Script | `config.js`의 `apiUrl` | 인증, Sheets 읽기·쓰기, 공개 범위 필터 | URL 응답은 정상이나 최신 v3 배포 미확인 |
| Google Sheets | 교사 개인 스프레드시트 | 모든 업무·공지·응답의 원본 | 앱 전용 탭 사용 |
...
- [ ] **P0** 최신 `apps-script.gs`를 실제 Apps Script 프로젝트에 반영하고 기존 웹앱을 새 버전으로 배포한다.
- [ ] **P0** 배포 후 `GET ?action=health`가 `{ ok: true, version: 3, service: "class-planner" }`를 반환하는지 확인한다. 2026-08-15 현재 운영 URL은 이 요청에 학생 피드를 반환한다.
- [ ] **P0** 개인 알림장 브라우저에서 클립보드의 브리지 접근 키를 입력하고 `연결 확인`을 완료한다.
- [ ] **P0** `[개인]`, `[교과]`, `[학급]`, `[학생개별]`을 실제 시트에 반영하고 관리자 게시부터 학생 표시·응답까지 종단 간 테스트한다.
