...
- 개발 내용
  ...
  - `DDISH_NM`의 `<br/>` 분리 후 알레르기 번호와 꼬리 점 제거(`하이라이스.. (1.2.5.6)` → `하이라이스`), `CAL_INFO`는 헤더 오른쪽에 표기
...
- 검증
  - 오늘 8/27: 하이라이스·계란파국·꽃맛살샐러드·수제함박떡스테이크·취나물볶음·깍두기·배스킨라빈스 / 810.4 kcal — 홈페이지 게시 내용과 일치
  - 8/25(중식+석식) 두 묶음 표시, 8/29(토)·1/5(방학) `오늘은 등록된 급식이 없습니다`, `fetch` 실패 주입 시 안내 + `다시 시도`로 정상 복구 확인
  - 레이아웃: 1280px 3칸(패널 338px), 1100px 이하 급식 카드 한 줄 차지, 375px 가로 스크롤 없음. 기존 테스트 7건 통과
- 남은 개발 항목
  - 나이스에 아직 안 올라온 날은 빈 값으로 나옴 — 필요하면 '이번 주' 조회로 바꿔 다음 급식일까지 당겨 보여줄 수 있음
  - 알레르기 정보를 뗀 상태라, 필요해지면 토글로 되살릴 것
- 가장 최근에 진행한 내용
  - `config.js`·`index.html`·`app.js`·`styles.css` 수정, 정적 자산 버전 `v=20260827-meal1`로 갱신
