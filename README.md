...
5. 관리자 화면에서 `위로`·`아래로` 버튼을 필요한 만큼 연속으로 눌러 전달 순서를 정하고, `순서가 저장되었습니다.`를 확인한 뒤 `게시하기`를 누릅니다.
...
번호 목록이나 서로 독립된 일정이 들어오면 Claude가 항목 사이를 `---`로 나누고 명령을 반복합니다. Apps Script는 여러 명령 블록과 `1.`, `2.` 형식의 목록을 각각 별도의 개인 알림장 행으로 저장하므로 항목마다 완료 체크할 수 있습니다. `[학급]`과 `[학생개별]` 항목은 검토함에도 각각 별도 초안으로 생성됩니다. 검토함에서 정한 순서는 Google Sheets에 저장되어 게시 후 학생 알림장에도 그대로 적용됩니다.
